#!/bin/bash
#
##setup command=wget https://github.com/emilnabil/download-plugins/raw/refs/heads/main/EstalkerWebControl/estalkerwebcontrol.sh -O - | /bin/sh

PLUGIN_URL="https://github.com/emilnabil/download-plugins/raw/refs/heads/main/EstalkerWebControl"

if [ -d /usr/lib64 ]; then
    PLUGINPATH="/usr/lib64/enigma2/python/Plugins/Extensions/EStalkerWebControl"
else
    PLUGINPATH="/usr/lib/enigma2/python/Plugins/Extensions/EStalkerWebControl"
fi
...
wget "$PLUGIN_URL/EstalkerWebControl.tar.gz" -O EstalkerWebControl.tar.gz > /dev/null 2>&1

if [ -f EstalkerWebControl.tar.gz ]; then
    echo "Extracting plugin..."
    tar -xzf EstalkerWebControl.tar.gz -C / > /dev/null 2>&1
    ...
    rm -f /tmp/EstalkerWebControl.tar.gz > /dev/null 2>&1
    sync
    ...
    echo "#########################################################"
    echo "# ERROR: Failed to download EstalkerWebControl.tar.gz   #"
    echo "#########################################################"
    exit 1
fi
