...
curl -k -L -m 60 "https://github.com/emilnabil/download-plugins/raw/refs/heads/main/spinners/spinner-fan.tar.gz" -o spinner-fan.tar.gz

if [ -s spinner-fan.tar.gz ]; then
    echo "Installing spinner..."
    tar -xzf spinner-fan.tar.gz -C / || { echo "❌ Extraction failed!"; exit 1; }
    sleep 1
