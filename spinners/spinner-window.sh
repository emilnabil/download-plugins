...
curl -k -L -m 60 "https://github.com/emilnabil/download-plugins/raw/refs/heads/main/spinners/spinner-window.tar.gz" -o spinner-window.tar.gz

if [ -s spinner-window.tar.gz ]; then
    echo "Installing spinner..."
    tar -xzf spinner-window.tar.gz -C / || { echo "❌ Extraction failed!"; exit 1; }
    sleep 1
    if [ -d /usr/share/enigma2/spinner ]; then
        cp -p /usr/share/enigma2/spinner/* /usr/share/enigma2/skin_default/spinner/ >/dev/null 2>&1
    fi
    rm -f spinner-window.tar.gz
