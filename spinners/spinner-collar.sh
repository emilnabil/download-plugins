...
curl -k -L -m 60 "https://github.com/emilnabil/download-plugins/raw/refs/heads/main/spinners/spinner-collar.tar.gz" -o spinner-collar.tar.gz

if [ -s spinner-collar.tar.gz ]; then
    echo "Installing spinner..."
    tar -xzf spinner-collar.tar.gz -C / || { echo "❌ Extraction failed!"; exit 1; }
    sleep 1
