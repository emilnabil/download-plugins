#!/bin/bash
#
### Command=curl -kLs https://github.com/emilnabil/download-plugins/raw/refs/heads/main/spinners/spinner-3D.sh | sh
#####################
...
curl -k -L -m 60 "https://github.com/emilnabil/download-plugins/raw/refs/heads/main/spinners/spinner-3D.tar.gz" -o spinner-3D.tar.gz

if [ -s spinner-3D.tar.gz ]; then
    echo "Installing..."
    tar -xzf spinner-3D.tar.gz -C / || { echo "Extraction failed!"; exit 1; }
    sleep 1
