...
curl -k -L -m 60 "https://github.com/emilnabil/download-plugins/raw/refs/heads/main/spinners/spinner-Kupler.tar.gz" -o spinner-Kupler.tar.gz

if [ -s spinner-Kupler.tar.gz ]; then
    echo "Installing spinner..."
    tar -xzf spinner-Kupler.tar.gz -C / || { echo "❌ Extraction failed!"; exit 1; }
    sleep 1
