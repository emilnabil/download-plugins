...
addFont("/usr/lib/enigma2/python/Plugins/Extensions/IpAudioPlus/fonts/digicon.ttf", "di", 100, 1)
addFont("/usr/lib/enigma2/python/Plugins/Extensions/IpAudioPlus/fonts/arial.ttf", "Regular", 100, 1)

try:
    from Plugins.Extensions.IpAudioPlus import apx
    version = apx.version
except:
    version = ""
rev = "_r010"
...
class ipaudioplus(Screen):
    ...
    def getAct(self):
        ...
        try:
            ...
            self.audiopluslis = []
        self['status'].hide()
        self['title'].setText(_("IpAudioPlus {}".format(version)))
        self.audioList()

    def load_channels_from_json(self):
        channels = []
        json_path = "/etc/enigma2/ipaudioplus.json"
        try:
    ...
    def audioList(self):
        try:
            self['audioSource'].setText(_("Audio Source : IpAudioPlus"))
            ...
            for i in range(len(self.list)):
                pcn1 = LoadPixmap(cached=False, path="/usr/lib/enigma2/python/Plugins/Extensions/IpAudioPlus/plugin.png")
                self.eventName = ""
                self.evnt(i)
            ...
            if config.plugins.ipap.updt.value == True:
                url = "https://api.github.com/repos/digiteng/APproject/releases/latest"
                r = urlopen(Request(url))
                rj = json.loads(r.read())
                latest_version = rj["name"]
                if version != latest_version:
                    up_msg = "new version available...\n\n{} to {}\n\ndo you want to update?..\n\n{}".format(version, latest_version, rj["body"])
    ...
    def instalUpdate(self, answer):
        try:
            if answer is True:
                cmd = ['wget -q "--no-check-certificate" https://github.com/digiteng/APproject/raw/main/ap.sh -O - | /bin/sh']
                self.session.open(Console, title='Updating Plugin...', cmdlist=cmd, finishedCallback=self.finished, closeOnSuccess=False)
        except Exception as err:
            self.errorlog(err)
            ...
            try:
                ...
                if os.path.exists(picon_name):
                    ...
                    if os.path.exists(picon_name):
                        ...
                        self.piconName = "/usr/lib/enigma2/python/Plugins/Extensions/IpAudioPlus/plugin.png"
                        return
            except:
                pass
        except Exception as err:
    ...
    def errorlog(self, err):
        ...
        with open("/tmp/IpAudioPlus.log", "a+") as f:
    ...
    def showImg(self):
        try:
            ...
            if curline == config.plugins.ipap.skin.value:
                picpath = "/usr/lib/enigma2/python/Plugins/Extensions/IpAudioPlus/images/preview/{}.jpg".format(curline)
    ...
    def update(self):
        try:
            ...
            if version != latest_version:
                ...
                self['info'].setText(_("NO NEW VERSION...Current Version {}".format(version)))
        except Exception as err:
            self.errorlog(err)
