#!/bin/sh
# setup command=wget https://github.com/emilnabil/download-plugins/raw/refs/heads/main/novaler/novalerStore.sh -O - | /bin/sh
########################################
...
case "$PY_VER" in
    "3.9"|"3.10"|"3.11"|"3.12"|"3.13"|"3.14") 
        PACKAGE="enigma2-plugin-extensions-novalerstore-py${PY_VER}_5.0-r0_all.ipk" 
        ;;
    *) 
...
SERVER_URL="http://ipk.ath.cx/pg/novalerstor"
...
if [ -s "$TMP_FILE" ]; then
    ...
    if [ "$FILE_SIZE" -gt 10240 ]; then
        ...
        if [ $? -eq 0 ]; then
            echo -e ">>> \033[0;32mDone! Plugin installed successfully.\033[0m"
