...
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpSettingsPanelModeEmil
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpSettingsPanelModeEmil
fi
...
if [ "$PYTHON" = "PY3" ] && ! grep -qs "Package: $Packagesix" "$STATUS"; then
    opkg update > /dev/null 2>&1 && opkg install $Packagesix -y > /dev/null 2>&1
fi
...
if ! grep -qs "Package: $Packagerequests" "$STATUS"; then
    ...
    if [ "$OSTYPE" = "DreamOs" ]; then
        ...
        opkg update > /dev/null 2>&1 && opkg install "$Packagerequests" -y > /dev/null 2>&1
    fi
fi
...
if [ "$PYTHON" = "PY3" ]; then
    wget -q "$PLUGIN_URL/py3/Ciefp-Panel.tar.gz"
else
    wget -q "$PLUGIN_URL/py2/Ciefp-Panel.tar.gz"
fi
